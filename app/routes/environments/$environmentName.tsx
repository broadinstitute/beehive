...
export const handle = {
    breadcrumb: () => {
        ...
        return <NavLink to={`/environments/${params.environmentName}`}>{params.environmentName}</NavLink>
    }
}

export const loader: LoaderFunction = async ({ request, params }) => {
    return new EnvironmentsApi(SherlockConfiguration)
        .apiV2EnvironmentsSelectorGet({ selector: params.environmentName || "" }, forwardIAP(request))
            ...
            <ViewPanel
                ...
                <div className="pt-4">
                    ...
                    {environment.defaultCluster ?
                        <p>Charts in this environment will deploy to the <Link to={`/clusters/${environment.defaultCluster}`} className="font-mono underline decoration-blue-500">{environment.defaultCluster}</Link> Kubernetes cluster by default.</p> :
                        <p>Charts in this environment don't have a default Kubernetes cluster, so one would need to be set manually per-chart to deploy anything for real.</p>}
                    <p>Charts in this environment will deploy to a <span className="font-mono">{environment.defaultNamespace}</span> Kubernetes namespace by default.</p>
                </div>
                <div className="pt-4">
                    <h2 className="text-2xl font-light">Access</h2>
                    <p>This environment is maintained by {environment.owner}.</p>
                    <p>This environment {environment.requiresSuitability ? "is" : "is not"} marked as requiring production suitability to modify.</p>
                </div>
