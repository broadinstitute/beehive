    ...
    return (
        <div className="flex flex-row h-full overflow-x-auto scroll-smooth">
            <ListPanel
                ...
                filter={(chart, filter) => chart.name?.includes(filter) || chart.chartRepo?.includes(filter)}
                borderClassName="border-sky-300"
                toCreateNew="/charts/new">
                {(chart) => (
                    <h2 className="font-light">{`${chart.chartRepo} / `}{<span className="font-medium">{chart.name}</span>}</h2>
                )}
            </ListPanel>
            <Outlet />
        </div>
