...
const DeletePanel: FunctionComponent<DeletePanelProps> = ({
  name,
  deleteGuard = true,
  borderClassName,
  backgroundClassName,
  ...
  <div className="w-[33vw] flex flex-col items-center space-y-4 pb-4">
    ...
    >
      {children}
      <p className="text-sm">
        DevOps can manually un-delete anything in Sherlock's database, keeping
        associations intact, but this is an emergency process.
      </p>
      ...
      <ActionButton borderClassName={borderClassName} type="submit">
        <h2 className="font-light">
          Click to Delete <span className="font-medium">{name}</span>
        </h2>
      </ActionButton>
    </Form>
  </div>
