...
const ListPanel = <T extends unknown>({
    title, entries, to, filter, children, borderClassName, toCreateNew
}: ListPanelProps<T>) => {
    ...
    return (
        <div className="w-[33vw] shrink-0 overflow-auto flex flex-col items-center space-y-4 pb-4">
            <div className={`text-left w-[30vw] bg-white rounded-2xl rounded-t-none border-2 border-t-0 p-3 pt-4 shadow-md ${borderClassName}`}>
                <h1 className="text-3xl font-medium">{title}</h1>
            </div>
            <div className="w-[30vw] flex flex-row-reverse">
                {toCreateNew && <LineNavButton to={toCreateNew} borderClassName={borderClassName} sizeClassName="w-[12vw] ml-4">
                    <h2 className="font-medium">Create New</h2></LineNavButton>}
                <input
                    type="text"
                    placeholder="Search..."
            ...
            {entryButtons.length > 0 ? entryButtons : <div>{`(There's nothing here${filterText === "" ? "" : `, matching "${filterText}"`})`}</div>}
        </div>
    )
}
